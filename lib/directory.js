  ...
  fs.readdir(self.path, function(err, files) {
    files.forEach(function(file) {
      ...
      fs.stat(path, function(err, stat) {
        err ?
          ee.emit('error', err) : stat.isFile() && self.parser.match(path) ?
          ee.emit('data', new self.file_class(path, self.parser)) :
          ee.emit('data', new Directory(path, self.parser, self.file_class));
      });
    });    
  });
  return ee; 
